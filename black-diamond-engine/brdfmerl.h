...
#ifndef black_diamond_engine_brdfmerl_h
#define black_diamond_engine_brdfmerl_h
...
public:
    std::vector<float> brdf(bdm::Vector sample, bdm::Vector view, bdm::Vector normal, std::string type) {
        
        std::vector<float> color(3);
        double* brdf;
        
        // read brdf
        if (!read_brdf(type.c_str(), brdf)) std::cout << "Error reading " << type << std::endl;
        
        double theta_in = acos(sample.dot(normal))*M_PI/180;
        double theta_out = acos(view.dot(normal))*M_PI/180;
        
        //std::cout << theta_in << " " << theta_out << std::endl;
        
        bdm::Transform t = t.rotate(normal,bdm::Vector(0,1,0));
        
        /*sample = t(sample);
        view = t(view);*/
        
        double temp1 = atan2(sample.x,sample.y);
        double temp2 = atan2(view.x,view.y);
        
        double phi_in;
        double phi_out;
        
        (temp1 < 0) ? phi_in = temp1 + 2*M_PI : phi_in = temp1; 
        (temp2 < 0) ? phi_out = temp2 + 2*M_PI : phi_out = temp2;
        
        phi_in *= M_PI/180;
        phi_out *= M_PI/180;
        
        //std::cout << phi_in << " " << phi_out << std::endl;
